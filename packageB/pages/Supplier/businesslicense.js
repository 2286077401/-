...
Page({
  ...
  previewImg: function (e) { 
    let str = this.data.listdata--
    let array = [];
    str.map(item => {
      let path=this.data.imgUrl+item.folder+item.autoname
      array.push(path)
    })   
    var index = this.data.imgUrl+e.currentTarget.dataset.imgdata.folder+e.currentTarget.dataset.imgdata.autoname; //当前图片地址
    var imgArr = array; //所有要预览的图片的地址集合 数组形式 
    wx.previewImage({
  ...
  imgupload: function () {
    ...
    for (var i = 0; i < tempFilePaths.length; i++) {
      wx.uploadFile({
        url: urlpath + '/purchase?action=addSupplierPhoto&supplierId=' + that.data.supplierId + '&photoType=yyzz',
