...
Page({
  ...
  submit: function () {
    ...
    let url = app.globalData.url + '/customer?action=getBeans&table=khxx&rows=100&page=1&mobile=true'
    let data = new Object()
      data.idEQ = this.data.khNameid,
      data.principalLIKE = this.data.ywyNameid,
      data.classidEQ = this.data.khclassNameid,
      data.tracelevelidEQ = this.data.khstateNameid,
      data.provinceEQ = this.data.provinceid,
      data.cityEQ = this.data.cityid,
      data.countyEQ = this.data.countiesid,
  ...
  //客户状态列表
  // 更多----查询条件 客户状态 接口NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/sysbase"];
  //   NSDictionary *params = @{@"action":@"getSelectType",@"type":@"manageclass"};
  khstateName: function () {
