...
Page({
  ...
  onLoad: function (options) { 
    if (options.showtype == '业务员产品销售分析数据统计搜索') {
      ...
      this.listData('', wx.getStorageSync('serchcustid'), '', '', '&isstat=true');

      return;
    }
    this.listData('', wx.getStorageSync('serchcustid'), '', '', '');
    this.listData('', wx.getStorageSync('serchcustid'), '', '', '&isstat=true');
    this.setData({
  ...
  next: function (proidEQ, custid, saletimeGE, saletimeLE) {
    ...
    page += 1
    let url = app.globalData.url + '/report?action=salerProDetail&rows=20&page=' + page
    let data = new Object()
    data.proidEQ = proidEQ
    data.salerid = custid
    data.saletimeGE = saletimeGE
  ...
  ranking: function () { 
    wx.navigateTo({
      url: '../custprosales?selarprosaleslistnext=' + JSON.stringify(this.data.selarprosaleslistnext),
    })

  },
  serch(e) { 
    let serchstate = '业务员产品销售分析数据统计搜索'
    // let serchstate = '客户产品销售分析数据统计详情搜索'
    wx.navigateTo({
      url: '../serchdate?serchstate=' + serchstate + '&barcustid=' + this.data.barcustid,
    })
  },
  /**
   * 生命周期函数--监听页面显示
  ...
  onReachBottom: function () {
    this.next('', wx.getStorageSync('serchcustid'), '', '', '');
